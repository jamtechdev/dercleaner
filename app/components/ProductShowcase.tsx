...
// Helper to validate if a string is a valid image URL/path
...
export default function ProductShowcase({ site }: { site: any }) {
  ...
  return (
    ...
    >
      ...
      {/* Main Content Area */}
      <div className="relative flex-1 flex flex-col md:flex-row gap-4 md:gap-8 md:gap-0 p-4 sm:p-6 md:p-16 w-full overflow-x-hidden md-mb-10">
        <div
          key={activeProduct.id}
          className="dc-animate-fade-up w-full flex flex-col md:flex-row gap-4 md:gap-8 md:items-center"
        >
          {/* Large Product Image */}
          {isValidImageSrc(activeProduct.heroImage?.src) ? (
          ...
          <div className="w-full md:w-1/2 space-y-4 md:space-y-10 flex flex-col">
            ...
            <div className="pt-2 md:pt-4 space-y-3 md:space-y-14 technishe-daten w-full">
              <button
                ...
                className="flex items-start text-gray-800 font-bold  flex-col hover:opacity-80 transition-opacity cursor-pointer w-full md:w-auto text-left technische-arrow"
              >
    ...
    >
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="min-w-0 flex-1" >
          <h3 className="font-bold text-sm md:text-base sm:text-lg truncate md:truncate-none pr-[30px]">{name}</h3>
          {!end && (
            <Image
  ...
  return (
    <div className="flex items-center p-3 md:p-4 rounded-2xl w-full bg-[var(--tertiary-color)]">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm md:text-[16px] lg:text-xl font-medium text-gray-800 leading-none pop-up-date  ">{value}</p>
      ...
      >
        ...
        <div className="p-4 md:p-6 lg:p-10">
          ...
          <div className={`relative w-full h-[200px] sm:h-[300px] md:h-[400px] lg:h-[490px] mb-4 md:mb-8 bg-gray-50 rounded-lg md:rounded-xl overflow-hidden imop-pop-box ${product.id}`}>
            {isValidImageSrc(product.featuresImage?.src) ? (
              <Image
              ...
              />
            ) : isValidImageSrc(product.video?.src) ? (
              <video
            ...
            ) : isValidImageSrc(product.heroImage?.src) ? (
              <Image
          ...
          {product.technicalSpecs && (
            // Check if it's the new format (object with items)
          ...
          {/* Features */}
          {product.features && (
            // Check if it's the new format (object with items) or old format (array)
