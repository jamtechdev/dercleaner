    ...
    return (
        <form action={saveSiteQuickAction} className="space-y-6 max-w-4xl">
            ...
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-800">FAQ / Kundenstimmen</h2>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                <h3 className="mb-4 text-base font-bold text-slate-800">Bereichsüberschrift</h3>
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-bold uppercase text-slate-400">Kontakt-Button-Text</label>
                        <input
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                <h3 className="mb-4 text-base font-bold text-slate-800">Einträge (Fragen oder Kundenstimmen)</h3>
                <div className="space-y-6">
                    {items.map((item: any, i: number) => (
                        <div key={i} className="flex flex-col gap-4 p-6 bg-slate-50 rounded-xl border border-slate-100 relative group">
                            ...
                            <div>
                                <label className="mb-1 block text-xs font-bold uppercase text-slate-400">Frage / Name</label>
                                <input
                                    name={`faqQuestion_${i}`}
                                    defaultValue={item.question ?? ""}
                                    placeholder="Frage / Name"
                            ...
                            <div>
                                <label className="mb-1 block text-xs font-bold uppercase text-slate-400">Antwort / Inhalt</label>
                                <RichTextEditor
                                    name={`faqAnswer_${i}`}
                                    defaultValue={item.answer ?? ""}
                                    placeholder="Antwort / Inhalt"
                                />
                            </div>
                        </div>
                    ))}
                    ...
                    {items.length === 0 && (
                        <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                            <p className="text-slate-400 text-sm">Noch keine Einträge. Klicken Sie auf „+ Eintrag hinzufügen“.</p>
                        </div>
                    )}
