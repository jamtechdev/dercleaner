    ...
    return (
        <form action={saveSiteQuickAction} className="space-y-6 max-w-4xl">
            ...
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-800">Navigation Menu</h2>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                <h3 className="mb-4 text-base font-bold text-slate-800">Menu Links</h3>
                <p className="mb-4 text-sm text-slate-500">Edit the main navigation links for the website header.</p>
                    ...
                    {links.map((link: any, i: number) => (
                        <div key={i} className="flex flex-col sm:flex-row gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100 items-center">
                            <div className="flex-1 w-full">
                                <label className="mb-1 block text-xs font-bold uppercase text-slate-400">Label {i + 1}</label>
                                <input
                                    name={`navLabel_${i}`}
