    ...
    return (
        <form action={saveSiteQuickAction} className="space-y-6 max-w-4xl">
            ...
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold text-slate-800">Funktionen-Bereich</h2>
            ...
            <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-100">
                <h3 className="mb-4 text-base font-bold text-slate-800">Funktionen verwalten</h3>
                <p className="mb-6 text-sm text-slate-500">Bearbeiten Sie die Funktionen auf der Startseite. Sie können beliebig viele hinzufügen.</p>
                ...
                <div className="space-y-8">
                    {features.map((feature: any, i: number) => (
                        <div key={i} className="p-6 bg-slate-50 rounded-2xl border border-slate-100 space-y-4 relative group">
                            ...
                            <h4 className="font-bold text-slate-800 uppercase text-xs tracking-wider">Funktion {i + 1}</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="mb-1 block text-xs font-bold uppercase text-slate-400">Icon-URL (SVG)</label>
                                    <input
                                        name={`featureIcon_${i}`}
                ...
                {features.length === 0 && (
                    <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        <p className="text-slate-400 text-sm">Noch keine Funktionen. Klicken Sie auf „+ Funktion hinzufügen“.</p>
                    </div>
                )}
